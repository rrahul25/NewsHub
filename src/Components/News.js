...
export class News extends Component {
  ...
  componentDidUpdate(prevProps) {
    // Check if category or country props have changed
    if (prevProps.category !== this.props.category || prevProps.country !== this.props.country) {
      // Reset state and fetch new articles
      this.setState(
        {
          articles: [],
          page: 1,
        ...
        () => {
          this.fetchArticles();
          // Update the document title for the new category
          document.title = `${this.capitalizeFirstLetter(this.props.category)} - NewsHub`;
        }
      );
    }
  ...
  fetchArticles = async () => {
    ...
    try {
      ...
      // Check if parsedData and parsedData.articles are valid
          ...
          <div className="container">
            <div className="row">
              {this.state.articles.map((element) => {
                // Ensure element has the required properties
                if (!element || !element.url) {
                  console.error('Unexpected article structure:', element);
                  return null; // Skip rendering this item if it's not valid
                }
